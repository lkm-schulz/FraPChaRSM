...
def process_host(host):
    ...
    for file in files:
        ...
        proc = subprocess.run(f'scp -o StrictHostKeyChecking=no -i {path_key} {file} {host.strip()}:~/', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        if proc.returncode != 0:
            return proc

    command = f"./{target.split('/')[-1]}" if mode == 'script' else target
    if args.verbose:
        print(f"ssh -o StrictHostKeyChecking=no -i {path_key} {host.strip()} {command}")
    proc = subprocess.run(f"ssh -o StrictHostKeyChecking=no -i {path_key} {host.strip()} {command}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
